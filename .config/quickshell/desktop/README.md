...
| `dashboard.showOnHover`, `dashboard.hotspot`, `dashboard.width` | top-centre panel (calendar, up next, notifications, now playing, usage): revealed when the pointer touches the top edge inside a `hotspot` px wide strip (default `true`, 480), or with `SUPER+G` / `qs -c desktop ipc call dashboard toggle`; panel width (default 1040). In frame style it grows out of the top band |
