...
| `bar.width`, `bar.pinThreshold`, `bar.showResources`, `bar.position`, `bar.entries` | bar thickness; drag distance that pins it; CPU/memory meters entry; `left`, `right` or `top`; entry order; available ids `kgrid`, `window`, `spacer`, `media`, `resources`, `tray`, `status`, `notifications`, `clock`, `power` (`window` is not in the default list) |
