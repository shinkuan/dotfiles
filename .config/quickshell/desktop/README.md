...
| `bar.width`, `bar.pinThreshold`, `bar.autoHide`, `bar.trayCompact`, `bar.showResources`, `bar.position`, `bar.entries` | bar thickness; drag distance that pins it (a drag on the bar pins it until dragged back or `bar unpin`); hide the bar until hovered (default `true`; `false` keeps it out); fold tray icons behind a chevron (default `true`, state remembered); CPU/memory meters entry; `left`, `right` or `top`; entry order; available ids `kgrid`, `window`, `spacer`, `media`, `resources`, `tray`, `status`, `notifications`, `clock`, `power` (`window` is not in the default list) |
